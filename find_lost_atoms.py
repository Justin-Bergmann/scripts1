...
#occ, b_fac, seg_i, ele, charge]
        ...
        string = string + str('{:3}'.format(res_num))
        string = string + "/g\" pdb"
        string = string + "\n"
        return string
...
def find_missing(pdb1,pdb2):
    miss1=[]
    miss2=[]
    miss3=[]
    print("len pdb1", len(pdb1))
    print("len pdb2", len(pdb2))
    print("number of missing atoms",abs(len(pdb1)-len(pdb2)))
    for i in range(len(pdb1)):
        ex=False
        ex2=False
        for k in range(len(pdb2)):
            if pdb1[i][6]==pdb2[k][6]:
              if pdb1[i][2]==pdb2[k][2]:
#              if pdb1[i][13]==pdb2[k][13]:
                  ex2=True 
            if pdb1[i][8]==pdb2[k][8]:
              if pdb1[i][9]==pdb2[k][9]:
               if pdb1[i][10]==pdb2[k][10]:
#                 print(pdb1[i])
#                 print(pdb2[k])
                  ex=True
#                 if pdb1[i][1]!= pdb2[k][1]:
#                    print(pdb1[i])
#                    print(pdb2[k])
                  if ex==False:
                     print(pdb1[i])
    ...
    for i in range(len(pdb2)):
        ex=False 
        for k in range(len(pdb1)):
            if pdb2[i][8]==pdb1[k][8]:
              if pdb2[i][9]==pdb1[k][9]:
               if pdb2[i][10]==pdb1[k][10]:
                  ex=True
        if ex==False:
               miss1.append(pdb2[i])
    for i in range(len(pdb1)):
        for k in range(len(pdb1)):
            if pdb1[i][8]==pdb1[k][8]:
              if pdb1[i][9]==pdb1[k][9]:
               if pdb1[i][10]==pdb1[k][10]:
                    if pdb1[i][1]!= pdb1[k][1]:
                       print(pdb1[i])
                       print(pdb1[k])
    print("the folowing atoms are missing in file 1")
    print(len(miss1))
    for i in range(len(miss1)):
        print(miss1[i])
    print(len(miss2))
    print("the folowing atoms are missing in file 2")
    for i in range(len(miss2)):
        print(miss2[i])

    print("the folowing atoms are missing in file 3")
    for i in range(len(miss3)):
        print(miss3[i])
...
org_pdb, pdb1, head = read_pdb(input_pdb)
org_pdb2, pdb2, head2 = read_pdb(input_pdb2)
print("first atom",pdb1[0])
find_missing(pdb1,pdb2)
